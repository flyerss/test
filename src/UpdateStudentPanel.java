...
public class UpdateStudentPanel extends JPanel {
    JTable table;
    String []head ={"学号","姓名","性别","年龄","电话家长姓名","所在年级","父母电话","家庭地址"};
    String [][] a ={{"","","","","","","",""}};
    JButton QueryButton;
    ...
    UpdateStudentPanel() {
        ...
        QueryButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                pre.setSQL("select * from student_info where Snumber ="+"'"+a[0][0].trim()+"'");
                pre.startQuery();
                String temp[][];
                temp=pre.getRecord();
                if(temp.length==0){
                    for(int i=1;i<a[0].length;i++){a[0][i]="";}
                    JOptionPane.showMessageDialog(null,"无此学生信息","提示", JOptionPane.WARNING_MESSAGE);
                    table =new JTable(a,head);
                    table.setFont(new Font("宋体",Font.BOLD,16));
                    table.setRowHeight(40);
        ...
        UpdateButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                handle.setSql("update student_info set Snumber=?,Sname=?,gender=?,age=?,parentname=?,gradenow=?,phonenumber=?,homeaddress=? where Snumber='"+a[0][0].trim()+"'");
                handle.setArray(a[0]);
                int m =handle.startInsert();
                if(m>0) {
                    JOptionPane.showMessageDialog(null,"修改学生成功","提示", JOptionPane.WARNING_MESSAGE);
                }
            }
        });
    }
